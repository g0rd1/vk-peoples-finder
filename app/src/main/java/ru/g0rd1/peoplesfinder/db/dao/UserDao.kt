...
abstract class UserDao : BaseDao<UserEntity>() {
    ...
    @Query(
        """
            SELECT * FROM (
                ...
                FROM `group` g JOIN `user_group` ug ON g.id = ug.group_id 
                GROUP BY user_id 
                ORDER BY COUNT(group_id) 
                DESC
            )
            as jn JOIN user ON jn.user_id = user.id 
            LIMIT :count 
            OFFSET :offset"""
    )
    ...
    @Query(
        """
            SELECT * FROM (
                SELECT user_type_id 
                FROM `user` JOIN `user_user_type` ON `user`.id = `user_user_type`.user_id
                WHERE `user`.id = :id
            ) 
            jn JOIN user_type ON jn.user_type_id = user_type.id"""
    )
